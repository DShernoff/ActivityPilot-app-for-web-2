...
import logging
import scheduler  # import our optimized scheduler module
...
def index():
    ...
    html = "<h1>Scheduler (Replit)</h1>"
    html += f"<p>Tasks: {s['tasks'].__len__()}</p>"
...
def add_task_route():
    ...
    # Accept only expected fields; extras are stored in metadata
    try:
...
def schedule_route():
    ...
    try:
        # Accept ISO datetimes or default to today 9:00->17:00
        ...
        if ds and de:
            ...
            # default workday today 9:00-17:00
            now = datetime.now()
        ...
        return jsonify({"status": "ok", "schedule": scheduler.export_schedule_json()["scheduled"]})
    except Exception as e:
...
if __name__ == "__main__":
    # Replit's default uses `python main.py` as the run command.
